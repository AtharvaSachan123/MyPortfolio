

export const navItems = [
  { name: "About", link: "#about" },
  { name: "Projects", link: "#projects" },
  { name: "Stack", link: "#stack" },
  { name: "Contact", link: "#contact" },
];

export const gridItems = [
  {
    id: 1,
    title: "I prioritize client collaboration, fostering open communication ",
    description: "",
    className: "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh]",
    imgClassName: "w-full h-full",
    titleClassName: "justify-end",
    img: "/b1.svg",
    spareImg: "",
  },
  {
    id: 2,
    title: "I'm very flexible with time zone communications",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "",
    spareImg: "",
  },
  {
    id: 3,
    title: "About Me",
    description: "I try to improve daily",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-center",
    img: "",
    spareImg: "",
  },
  {
    id: 4,
    title: "Tech enthusiast with a passion for development.",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },

  {
    id: 5,
    title: "Currently building a SaaS product for the web.",
    description: "The Inside Story",
    className: "md:col-span-3 md:row-span-2",
    imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "/b5.svg",
    spareImg: "/grid.svg",
  },
  {
    id: 6,
    title: "Do you want to start a project together?",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-center md:max-w-full max-w-60 text-center",
    img: "",
    spareImg: "",
  },
];

export const projects = [
  {
    id: 1,
    title: "FigPro",
    des: "A Figma clone that helps you design and prototype faster with components!",
    img: "/figpro.png",
    iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/next.svg"],
    link:"https://fig-proo.vercel.app/",
  },
  {
    id: 2,
    title: "CryptoHunt",
    des: "A real-time cryptocurrency tracker that helps you hunt the best deals!",
    img: "/crpyto.png",
    iconLists: ["/re.svg", "/tail.svg", "/js.svg", "/c.svg"],
    link: "https://crypto-hunt1.vercel.app/",
  },
  {
    id: 3,
    title: "Imaginify",
    des: "A REAL Software-as-a-Service app with AI features and a payments and credits system using the latest tech stack.",
    img: "/p3.svg",
    iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
    link: "",
  },
  {
    id: 4,
    title: "CodeX",
    des: "A code editor that helps you write and test your code in real-time with a live preview.",
    img: "/editor.png",
    iconLists: ["/re.svg", "/tail.svg", "/js.svg"],
    link: "https://code-editor11.vercel.app/",
  },
];
export const testimonials = [
  {
    
    img:"/js.svg",
  },
  {
    
    img:"/ts.svg"
  },
  {
    
    img:"/tail.svg"
  },
  {
    
    img:"/re.svg"
  },
  {
    
    img:"/html.svg"
  },
  {
    
    img:"/css.svg"
  },
  {
    
    img:"/next.svg"
  },
  {
    
    img:"/github.svg"
  },
  {
    
    img:"/java.svg"
  },
  {
    
    img:"/firebase.svg"
  },
  {
    
    img:"/three.svg"
  },
  {
    
    img:"/c.svg"
  },
  {
    
    img:"/mongo.svg"
  },
];

export const companies = [
  {
    id: 1,
    name: "cloudinary",
    img: "/cloud.svg",
    nameImg: "/cloudName.svg",
  },
  {
    id: 2,
    name: "appwrite",
    img: "/app.svg",
    nameImg: "/appName.svg",
  },
  {
    id: 3,
    name: "HOSTINGER",
    img: "/host.svg",
    nameImg: "/hostName.svg",
  },
  {
    id: 4,
    name: "stream",
    img: "/s.svg",
    nameImg: "/streamName.svg",
  },
  {
    id: 5,
    name: "docker.",
    img: "/dock.svg",
    nameImg: "/dockerName.svg",
  },
];

export const workExperience = [
  {
    id: 1,
    title: "Frontend Developer Intern",
    desc: " To Assist in the development of a web-based platform using React.js",
    className: "md:col-span-2",
    thumbnail: "/exp1.svg",
  },
  {
    id: 2,
    title: "Software Engineer Intern",
    desc: "My knowledge of DSA could help me to solve real-world problems.",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp2.svg",
  },
  {
    id: 3,
    title: "Freelance Web Dev Project",
    desc: "To develop a website for a client using Web Technologies.",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp3.svg",
  },
  {
    id: 4,
    title: "Full Stack Developer",
    desc: "To develop a full-stack application using the MERN stack.",
    className: "md:col-span-2",
    thumbnail: "/exp4.svg",
  },
];

export const socialMedia = [
  {
    id: 1,
    img: "/git.svg",
    link:"https://github.com/AtharvaSachan123"
  },
  {
    id: 2,
    img: "/twit.svg",
    link:"https://x.com/Atharva_Sachan_"
  },
  {
    id: 3,
    img: "/link.svg",
    link:"https://www.linkedin.com/in/atharva-sachan-975848252/"
  },
  {
    id: 4,
    img: "/cv.svg",
    link:"/AtharvaSachan_resume1.pdf",

    download:true
  },
];
